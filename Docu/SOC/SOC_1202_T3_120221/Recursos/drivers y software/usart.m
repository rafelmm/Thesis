...
while(~fin)
    ...
    elseif strfind(received,'Operacion: 0')==1 
            ...
            received = fscanf(s);
            while(strfind(received,'L'))
                received = fscanf(s);
            end
            received = fscanf(s);
            
            [width,height]=strtok(received);
            width = str2double(width);
            height = str2double(height);
            
            fin_receive_image = false;
            data = zeros(width*height,2);
            i=1;
            while(~fin_receive_image)
                ...
                elseif (strfind(received,'Fin')==1)
                    ...
                    i = i+1;
                    if mod(i,500)==0
                        display(['Received: ' num2str(floor(i/384)) '%']);
                    end
            ...
            close all
            I = display_image('usart',data,width,height,1);
            fprintf(s,'%c','n');
            fin = 1;
