...
/* Includes ------------------------------------------------------------------*/
#include "../inc/SM_stm32f4xx_iwdg.h"
#include "stm32f4xx.h"
...
/**
 * @brief  	Configura el watchdog para que se resetea en el intervalo especificado por cfg
 * @param	cfg Tiemout de watchdog
 * @retval	None
 */
void SM_IWDG_Config(SM_IWDG_Timeout_Cfg_t cfg){
	//Seteamos el bit que para el counter del watchdog cuando se para el sistema en un breakpoint
	DBGMCU->APB1FZ |= DBGMCU_APB1_FZ_DBG_IWDG_STOP;

	IWDG_WriteAccessCmd(IWDG_WriteAccess_Enable);
	IWDG_SetPrescaler(cfg.IWDG_Prescaler);
	IWDG_SetReload(cfg.IWDG_Reload);
	IWDG_ReloadCounter();
	IWDG_Enable();
}
